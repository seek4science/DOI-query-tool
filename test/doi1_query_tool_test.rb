...
class DoiQueryToolTest < Test::Unit::TestCase
  ...
  def test_book_chapter_doi
    VCR.use_cassette('fetch_comp_sci_book_chapter_doi') do
      ...
      assert_equal 'Sharing Data, Information and Knowledge,pages:87-99,Springer Berlin Heidelberg. 2008', result.citation
  ...
  def test_book_chapter_doi_2
    VCR.use_cassette('fetch_book_chapter_doi') do
      ...
      assert_equal 'Research and Advanced Technology for Digital Libraries,pages:300-311,Springer International Publishing. 2017', result.citation
  ...
  def test_inproceedings_doi
    VCR.use_cassette('fetch_inproceedings_doi') do
      ...
      assert_equal 'Developments in X-Ray Tomography XI,page 24,SPIE.2017', result.citation
  ...
  def test_any_doi
    VCR.use_cassette('temp_doi') do
      result = @client.fetch('10.3390/proceedings2019025039')
      puts "doi:"+result.doi
  ...
  def test_proceedings_doi_journal
    VCR.use_cassette('fetch_proceedings_doi') do
      result = @client.fetch('10.3390/proceedings2019025039')
      assert_equal :journal, result.publication_type
      assert_equal'Proceedings', result.journal
      assert_equal 'Proceedings 25(1) : 39. 2019', result.citation
      assert_equal 2, result.authors.size
      assert_equal '2019-01-01',result.date_published.to_s
